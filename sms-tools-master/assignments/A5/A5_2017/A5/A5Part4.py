...
def selectFlatPhasePeak(pX, p, phaseDevThres):
    ...
    """
    #Your code here
    x_mean = pX[p-2]+pX[p-1]+pX[p]+pX[p+1]+pX[p+2]
    standard_deviation = np.sqrt((np.square(pX[p-2]-x_mean)+np.square(pX[p-1]-x_mean)+np.square(pX[p]-x_mean)+np.square(pX[p+1]-x_mean)+np.square(pX[p+2]-x_mean))/5)
    flag = True
    if(standard_deviation<phaseDevThres):
        flag = True
    else:
        flag = False
    return flag
